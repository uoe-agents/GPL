...
onlyfiles3 = [f for f in listdir(mypath3) if isfile(join(mypath3, f)) and ".csv" in f]
...
onlyfiles7 = [f for f in listdir(mypath7) if isfile(join(mypath7, f)) and ".csv" in f]
onlyfiles8 = [f for f in listdir(mypath8) if isfile(join(mypath8, f)) and ".csv" in f]
...
min_len1, min_len2, min_len3, min_len4 = min([len(a) for a in raw_data_1]), min([len(a) for a in raw_data_2]), min([len(a) for a in raw_data_3]), min([len(a) for a in raw_data_4])
min_len5, min_len6, min_len7, min_len8 = min([len(a) for a in raw_data_5]), min([len(a) for a in raw_data_6]), min([len(a) for a in raw_data_7]), min([len(a) for a in raw_data_8])
min_len_all = min ([min_len1, min_len2, min_len3, min_len4, min_len5, min_len6, min_len7, min_len8])

raw_data_1 = [a[:min_len_all] for a in raw_data_1]
raw_data_2 = [a[:min_len_all] for a in raw_data_2]
raw_data_3 = [a[:min_len_all] for a in raw_data_3]
raw_data_4 = [a[:min_len_all] for a in raw_data_4]
raw_data_5 = [a[:min_len_all] for a in raw_data_5]
raw_data_6 = [a[:min_len_all] for a in raw_data_6]
raw_data_7 = [a[:min_len_all] for a in raw_data_7]
raw_data_8 = [a[:min_len_all] for a in raw_data_8]
...
plt.title("Average Training Return in Wolfpack")
...
g =sns.lineplot(x="Total Steps (x160000)", y="Average Return per Episode", data=df3, label="QL-RFM")
...
g =sns.lineplot(x="Total Steps (x160000)", y="Average Return per Episode", data=df5, label="GNN-RFM")
...
lgd = g.legend(bbox_to_anchor=(1.02, 1), fontsize = 'x-small')
plt.savefig("05AdhocTrain.pdf", bbox_inches='tight')
