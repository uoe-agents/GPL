...
class WolfpackPenaltySingle(gym.Env):
    ...
    def sample_teammates(self, num_sampled):
        poss_teammates = [RandomAgent, GreedyPredatorAgent, GreedyProbabilisticAgent,
                          TeammateAwarePredator, DistilledCoopAStarAgent, GreedyWaitingAgent, 
                          GreedyProbabilisticWaitingAgent, TeammateAwareWaitingAgent]

        return [poss_teammates[np.random.choice(range(len(poss_teammates)), 1)[0]](idx+1) for idx in range(num_sampled)]
