...
from Wolfpack_gym.envs.wolfpack_penalty_single_assets.ReplayMemory import ReplayMemoryLite
from Wolfpack_gym.envs.wolfpack_penalty_single_assets.QNetwork import DQN, RFMLSTMMiddle, GraphOppoModel
from Wolfpack_gym.envs.wolfpack_penalty_single_assets.misc import hard_copy, soft_copy
import torch
...
class GraphDQNAgent(Agent):
    ...
    def prep(self, obs, hiddens):
        ...
        for ob in obs:
            ...
            graph.append(graph_ob)

        graph = dgl.batch(graph)

        n_ob = torch.cat([torch.Tensor(elem[:2])[None, :].float() for a in obs for elem in a], dim=0)
        e_ob = torch.zeros([graph.number_of_edges(), 0])
        u_ob = torch.cat([torch.Tensor(k[0][2:]).float()[None, :] for k in obs], dim=0)

        if hiddens is None:
            n_hid = (torch.zeros([1, graph.number_of_nodes(), self.dim_lstm_out]),
                    torch.zeros([1, graph.number_of_nodes(), self.dim_lstm_out]))
            e_hid = (torch.zeros([1, graph.number_of_edges(), self.dim_lstm_out]),
                    torch.zeros([1, graph.number_of_edges(), self.dim_lstm_out]))
        ...
        if not self.prev_graph is None:
            if not self.remaining_indices is None:
                remaining_indices = np.where(self.remaining_indices != -1)[0].tolist()
                n_hid = (n_hid[0][:, remaining_indices, :], n_hid[1][:, remaining_indices, :])
                edges_src, edges_dst = zip(*[(i,j) for i in remaining_indices
                                             for j in remaining_indices if i != j])

                edge_ids = self.prev_graph.edge_ids(list(edges_src), list(edges_dst))
                e_hid = (e_hid[0][:, edge_ids, :], e_hid[1][:, edge_ids, :])

            if graph.number_of_nodes() - n_hid[0].shape[1] > 0:
                added_nodes = graph.number_of_nodes() - n_hid[0].shape[1]
                fin_num_edge = (graph.number_of_nodes() *(graph.number_of_nodes()-1))
                prev_num_edge = (n_hid[0].shape[1] * (n_hid[0].shape[1]-1))
                n_hid = (torch.cat([n_hid[0], torch.zeros([1,added_nodes, self.dim_lstm_out])], dim=1),
                         torch.cat([n_hid[1], torch.zeros([1,added_nodes, self.dim_lstm_out])], dim=1))
                e_hid = (torch.cat([e_hid[0], torch.zeros([1,fin_num_edge - prev_num_edge, self.dim_lstm_out])], dim=1),
                         torch.cat([e_hid[1], torch.zeros([1,fin_num_edge - prev_num_edge, self.dim_lstm_out])], dim=1))

        return graph, e_ob, n_ob, u_ob, e_hid, n_hid, u_hid
