...
class WolfpackPenalty(gym.Env):
    ...
    def __init__(self, grid_height=20, grid_width=20, num_players=5, max_food_num=2,
        ...
        self.num_players = np.random.choice(self.max_num_players-3, 1)[0] + 3
        self.action_space = spaces.Tuple([spaces.Discrete(self.N_DISCRETE_ACTIONS) for _ in range(self.num_players)])
        ...
        if obs_type == "vector":
            ...
            self.observation_space = spaces.Dict({'obs' : spaces.Box(low=np.array(box_low),
                                                              high=np.array(box_high),
                                                              dtype=np.int32) for _ in range(self.num_players)})
    ...
    def reset(self):
        ...
        if self.obs_type == "vector":
            ...
            for idx in range(6 * self.max_food_num):
                if idx % 6 == 0:
                    box_high.append(self.grid_height - 1)
                elif idx % 6 == 1:
                    box_high.append(self.grid_width - 1)
            ...
            box_low = [0] * len(box_high)
            self.observation_space = spaces.Dict({'obs': spaces.Tuple([spaces.Box(low=np.array(box_low),
                                                                    high=np.array(box_high),
                                                                    dtype=np.int32) for _ in range(self.num_players)])})
    ...
    def step(self, action):
        ...
        player_returns = ({'obs' : tuple([self.observation_computation(obs_type, agent_id=id)
