...
from Wolfpack_gym.envs.wolfpack_single_adhoc_assets.Agents import *
...
class WolfpackSingleAdhoc(gym.Env):
    ...
    def __init__(self, grid_height=20, grid_width=20, num_players=5, max_food_num=2,
        ...
        self.scheduler = OpenScheduler(self.num_players-1, self.add_rate, self.del_rate, self.max_player_num-1)
        self.masking = []
    ...
    def observation_computation(self, obs_type, agent_type="player", agent_id=0):
        ...
        if obs_type == "main_player":
            ...
            for player_locs in self.player_positions:
                teammate_locations.append(player_locs[0])
                teammate_locations.append(player_locs[1])

            teammate_locations.extend([-1]*(2*(self.max_player_num-self.num_players)))
...
class OpenScheduler(object):
    def __init__(self, num_agents, add_rate, remove_rate, max_available_agents):
    ...
    def agent_type_sampler(self):
        all_agent_types = [RandomAgent, GreedyPredatorAgent, GreedyProbabilisticAgent,
                          TeammateAwarePredator, DistilledCoopAStarAgent, GraphDQNAgent]
        agent_id_type = random.randint(0, len(all_agent_types)-1)
        return all_agent_types[agent_id_type]

    def agent_removal_sampler(self):
        removed_amount = min(np.random.choice(2, 1, p=[0.7, 0.3])[0] + 1, self.available_agents-1)
        removed_indices = []
    ...
    def open_process(self):
        ...
        if add:
            agent_nums = min(np.random.choice(2, 1, p=[0.7, 0.3])[0] + 1,
