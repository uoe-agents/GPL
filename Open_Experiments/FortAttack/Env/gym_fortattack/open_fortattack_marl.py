...
def make_open_fortattack_env_marl(args, num_steps, team_size, active_agents, num_freeze_steps, benchmark=False, seed=100,
                             reward_scheme="normal", team_mode=None):
    ...
    if benchmark:
        env = MARLOpenFortAttackGlobalEnv(
        ...
        env = MARLOpenFortAttackGlobalEnv(
            ...
            arguments=args, seed=seed, with_oppo_modelling=True, team_mode=team_mode
        )
    return env

class MARLOpenFortAttackGlobalEnv(gym.Env):
    metadata = {
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 ...
                 seed=100, with_oppo_modelling=False, team_mode=None):
        ...
        for agent in self.agents:
            ...
            self.observation_space_real.append(spaces.Box(low=-np.inf, high=+np.inf, shape=(obs_dim,), dtype=np.float32))
            agent.action.c = np.zeros(self.world.dim_c)

        # action has 8 values:
        # nothing, +forcex, -forcex, +forcey, -forcey, +rot, -rot, shoot
        self.action_space = Box(low=0., high=1., shape=((world.dim_p) * 2 + 2,))  ##
        self.observation_spaces = Box(low=-np.inf, high=+np.inf, shape=(self.world.numAgents, obs_shapes[0][0]+2))

        if self.with_oppo_modelling:
            self.observation_spaces = Box(low=-np.inf, high=+np.inf, shape=(self.world.numAgents, obs_shapes[0][0] + 3))
    ...
    def action_space(self):
        return [spaces.Discrete(9) for _ in range(self.agent_num // 2)]

    @property
    def observation_space(self):
        obs_shape = self.observation_space_real[0].shape[0]
        num_agents_per_team = len(obs_shape) // 2
        return [
            spaces.Box(low=-np.inf, high=+np.inf, shape=(obs_shape*(num_agents_per_team+1),), dtype=np.float32)
        ]
    ...
    def setup_master(self):
        ...
        pol_obs_dim = self.observation_space_real[0].shape[0]  ##* ensure 27 is correct

        pos_index = 2  ##* don't know why it's a const and +2

        for i, agent in enumerate(self.world.policy_agents):
            obs_dim = self.observation_space_real[i].shape[0]
    ...
    def step(self, action_n):
        ...
        teammate_actions[list(range(self.n//2))] = action_n
        ...
        done = [self._get_done() for _ in range(len(self.agents))]
        ...
        swapped_data = np.concatenate((swapped_data, self.add_team_data(swapped_data)), axis=-1)

        swapped_data_teammate = swapped_data[:(self.agent_num // 2)]
        obs_opponent = np.repeat(swapped_data[(self.agent_num // 2):].reshape(1, -1), self.agent_num // 2, axis=0)
        concat_obs_n = np.concatenate([swapped_data_teammate, obs_opponent], axis=-1)

        rew_tens = torch.from_numpy(np.stack(reward_n)).float()
        self.master.update_rollout(self.teammate_obs, rew_tens, masks)
        self.step_num += 1

        return concat_obs_n, reward_n[:(self.agent_num // 2)], done[:(self.agent_num // 2)], info_n['n'][:(self.agent_num // 2)]
